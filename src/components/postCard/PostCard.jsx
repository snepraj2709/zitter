...
export function PostCard({ post }) {
  ...
  return (
    <div className="flex flex-col shadow-sm p-1 md:p-2 cursor-pointer min-w-full border border-b-gray-300">
      <div>
        <div onClick={() => navigate(`/profile/${postByUser.username}`)} className='flex flex-row justify-between items-center'>
          <div className='flex flex-row m-2 items-center' onClick={() => navigate(`/profile/${username}`)}>
            ...
            <div className='flex flex-col ml-2 '>
              <span className="text-sm font-medium">{postByUser?.firstName} {postByUser?.lastName}</span>
              <span className="text-xs text-gray-500">@{postByUser?.username}</span>
            </div>
        ...
        <div className="flex flex-col justify-start mt-4 ml-10 py-3">
          ...
          >
            ...
            <p className="text-sm font-normal mt-2 break-all">{content}</p>
          </div>
